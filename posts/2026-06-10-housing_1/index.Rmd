...
selected_countries <- c("Ireland", "EU27", "France", "Germany", "Spain", "Italy",
                        "Malta", "Poland", "Belgium", "Denmark", "Sweden",
...
ggplot(bump_data_filtered, aes(x = year, y = rank, color = geo)) +
  ...
  scale_x_continuous(
    breaks = seq(min(bump_data_filtered$year), max(bump_data_filtered$year), 1),
  ...
  labs(
    ...
    x = "Year",
    y = "Rank",
    caption = "*Sources: Developer Value Margin (GOS/GVA), EU-27 and Ireland GOS/GVA: Eurostat sbs_ovw_act indicator GOS_AV_PC, NACE F41.1, updated 10/03/2026. <br> — Not all countries shown for display purposes*"
  ) +
  theme(
...
The GMI data for NACE 411 runs at between €22 million and €40 million per year across the entire 2015–2024 period. In 2024 it was approximately €22 million. Against a GOS of €949 million, GMI represents just 2.4% of total surplus generated. The overwhelming majority (97.6%) of the value extracted by NACE 411 flows to corporate entities, not self-employed individuals.
...
### 3.2  The Financing Cost Objection: The Second Extraction Layer
 
A serious objection to the headline finding deserves its own section, because it's methodologically correct as far as it goes. Gross operating surplus is measured before interest. GOS/GVA of 81% therefore describes what the production process yields to capital as a class; it doesn't describe what the developer's shareholders ultimately keep, because debt service is paid out of GOS, not before it. And Irish development finance is genuinely expensive at the smaller end of the market. Since 2008, the pillar banks have rationed residential development lending and international banks lend mainly to existing clients; the gap is filled by alternative lenders (credit funds, private equity debt vehicles, mezzanine providers, very largely foreign-capitalised) offering up to 90% loan-to-cost at margins in excess of 10%, plus arrangement fees and in some cases equity participation. Even Home Building Finance Ireland, the state's own remedial lender created in 2019 precisely because the private market wouldn't serve small builders, prices its small-development product at margins of 4.75% to 7.5% over Euribor. A small Irish developer's marginal capital can cost 10–15% all-in. The objection follows: perhaps a material share of the 81% isn't developer profit at all, but interest flowing onward to financiers. A different extractor, and one that might warrant more sympathy for the developer squeezed between them.
 
The objection survives as a refinement and fails as a rebuttal, for three reasons of magnitude. First, the industry's own cost accounting bounds it: the SCSI's December 2025 apartment cost stack prices total development finance at €31,000 per unit (6% of all-in cost) against a required developer margin of €64,710, or 12%. On the industry's own figures, the financiers' take is roughly half the developer's, and the SCSI viability hurdle is computed after the financiers are paid. Second, the European comparison is like-for-like: every country's F411 GOS in the Eurostat data is equally pre-interest, and continental developers also borrow. For financing-cost differentials alone to close the 19-point gap between Ireland's 81% and the EU average of 62%, the Irish sub-sector would need to carry several billion euro of debt at a sustained five-point rate premium over European peers — implausible when HBFI and senior bank facilities price far below the mezzanine rates that attract the headlines. Third, the firms this report actually documents are immune to the objection: Glenveagh's FY2025 accounts show gross profit of €198 million against profit before tax of €125 million, and that €73 million gap contains all administrative expenses as well as all finance costs. The expensive-money story is real, but it belongs to the small developer, not to the listed landbank holders whose margins anchor this report.
 
Properly understood, the objection strengthens the structural case in three ways. First, the financier's coupon is priced on the land-heavy capital requirement: the planning rent is the principal on which the interest is charged. Without €643,000-per-acre land there's no €31,000-per-unit finance line. The second extraction layer exists because the first one does, and a credit fund earning 12% on a land-dominated facility is collecting a share of the same planning rent by another name. Since these funds are very largely foreign-capitalised, this is also the most concrete channel through which the extraction documented in this report leaves Ireland — a channel no published statistic permits anyone to quantify. Second, the objection vindicates the four-sides analysis: HBFI exists because the banks withdrew, and the state's own remedial lender pricing at 7–10% all-in against a rent the state itself created is the four-sides square drawn in miniature. Third, it sharpens a distinction worth making explicit: the extraction is concentrated at the land-and-permission layer and among the large holders of cheap landbanks. The small developer paying mezzanine rates is squeezed from both sides, the landowner's rent above and the financier's coupon below, which is entirely consistent with the SPV-dominated, 1.7-persons-per-enterprise structure documented earlier, and which means the policy interventions in the final post would benefit small builders most: capture the rent or abolish its scarcity, and both the land bill and the coupon priced on it collapse together.
 
> *Sources: GOS definition (pre-interest): ESA 2010, §8.18–8.22; Eurostat SBS methodology. Alternative lender terms: Deloitte Ireland, Residential Development Funding Landscape (unitranche/mezzanine margins in excess of 10%, up to 90% LTC). HBFI Small Development Product: hbfi.ie (margin 4.75–7.5% over 3-month Euribor, 80% LTC); HBFI lending since 2019: €3bn approved, 15,000 homes (gov.ie, Budget 2026). Bank caution and international lender behaviour: Department of Finance, Report on the Availability, Composition and Flow of Finance for Residential Development. SCSI finance line: Real Costs of New Apartment Delivery 2025, Figure 3. Glenveagh: Full Year Results 2025.*
